...
// Register Admin (Restricted to Super Admin)
...
export const loginAdmin = async (req, res) => {
  try {
    ...
    // Generate real JWT with typed Admin role
...
export const getAllStudents = async (req, res) => {
  try {
    ...
    if (q) {
      ...
      query.$or = [{ name: regex }, { email: regex }, { branch: regex }, { rollNumber: regex }];
    }
...
export const createCollege = async (req, res) => {
  try {
    const { name, email, phone, password, university, code, address, city, state, website } = req.body;
    ...
    const college = await College.create({
      ...
      phone: phone.trim(),
      password,
      university: university ? university.trim() : "Affiliated University",
...
export const updateProject = async (req, res) => {
  try {
    ...
    ).populate("company recruiter college");
...
export const verifyCollege = async (req, res) => {
  try {
    ...
    const status = req.body.status || req.body.verificationStatus; // status: 'Verified' | 'Rejected'
    const note = req.body.note || req.body.verificationNote;
...
export const getPlacementOversight = async (req, res) => {
  try {
    const [
      ...
      PlacementDrive.countDocuments({
        status: { $in: ["Upcoming", "Ongoing", "REGISTRATION OPEN", "INTERVIEWING", "TESTING STAGE"] },
      }),
      Project.countDocuments({ status: "Open" }),
      College.countDocuments({ status: "Active" }),
      Application.countDocuments(),
      Application.countDocuments({ status: { $in: ["Offered", "Selected", "Placed"] } }),
      PlacementDrive.find()
        .populate("college", "name city code")
        .populate("company", "name industry location")
    ...
    // Attach application counts for PlacementDrive items
    const drivesWithStats = await Promise.all(
      placementDrivesList.map(async (pd) => {
        const applicantsCount = await Application.countDocuments({
          $or: [{ placementDrive: pd._id }, { drive: pd._id }],
        });
        return {
          _id: pd._id,
          title: `${pd.jobRole} (${pd.companyName})`,
          company: { name: pd.companyName },
          location: pd.location || "Campus",
          mode: pd.mode || "Virtual",
          openings: pd.capacityLimit || 0,
          stipend: typeof pd.packageLPA === "number" ? pd.packageLPA : parseFloat(pd.packageLPA) || 0,
          status: pd.status,
          approvalStatus: "Approved",
          applicantsCount: applicantsCount || pd.appliedCount || 0,
          type: "PlacementDrive",
    ...
    // Attach application counts for Project items
    const projectsWithStats = await Promise.all(
      projectsList.map(async (proj) => {
        ...
        return {
          _id: proj._id,
          title: proj.title,
          company: proj.company || { name: "Corporate Partner" },
          location: proj.location || "Remote",
          mode: proj.mode || "Remote",
          openings: proj.openings || 1,
          stipend: proj.stipend || 0,
          status: proj.status,
    ...
    const unifiedDrives = [...drivesWithStats, ...projectsWithStats]
    ...
    return successResponse(
      ...
      {
        summary: {
          activeDrives: activePlacementDrivesCount + activeProjectsCount,
...
export const createSupportTicket = async (req, res) => {
  try {
    ...
    const { requesterName, requesterEmail, requesterRole, priority, type } = req.body;
    ...
    if (requesterRole) {
      const r = requesterRole.toString().toLowerCase();
      if (r === "student") normalizedRole = "Student";
      else if (r === "college") normalizedRole = "College";
      else if (r === "recruiter") normalizedRole = "Recruiter";
      else if (r === "mentor") normalizedRole = "Mentor";
      else normalizedRole = requesterRole;
    }
...
export const replySupportTicket = async (req, res) => {
  try {
    ...
    const isInternalNote = req.body.isInternalNote || false;
    if (!text || !text.trim()) {
...
export const getPlatformAnalytics = async (req, res) => {
  try {
    const [
      ...
      Application.countDocuments({ status: { $in: ["Interview", "Interviewed"] } }),
      Application.countDocuments({ status: { $in: ["Offered", "Selected", "Placed"] } }),
    ]);
  ...
  try {
    ...
    if (!settings) {
      settings = await SystemSetting.create({
        ...
        maintenanceMode: {
          enabled: false,
          message: "System maintenance in progress. Only Super Admins retain access.",
        },
        apiIntegrations: {
          linkedIn: true,
          youTube: true,
