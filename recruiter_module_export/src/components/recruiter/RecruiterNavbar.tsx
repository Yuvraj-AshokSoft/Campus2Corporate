...
const RecruiterNavbar = ({ title, subtitle }: RecruiterNavbarProps) => {
  ...
  const switchTestAccount = (mode: "VERIFIED" | "UNFILLED") => {
    localStorage.setItem("c2c_verification_status", mode);
    if (mode === "VERIFIED") {
      const verifiedProfile = {
        repName: "Alexander Wright",
        companyName: "Lumina Nexus Tech",
        websiteUrl: "https://luminanexus.com",
        aboutCompany: "Enterprise AI software and campus recruitment platform partner.",
      ...
      localStorage.setItem("c2c_company_profile", JSON.stringify(verifiedProfile));
      pushRecruiterNotification({
        type: "admin",
        title: "Company Verification Approved 🎉",
        message: "Congratulations! Admin has verified your corporate profile. You have full access to publish jobs and shortlist talent."
      });
      window.location.href = "/recruiter/dashboard";
    } else if (mode === "UNFILLED") {
      localStorage.removeItem("c2c_company_profile");
      pushRecruiterNotification({
        type: "admin",
        title: "Account Unverified — Verification Required ⚠️",
        message: "Action Required: Please complete your company profile and upload a corporate logo in Settings to request Admin verification."
      });
      window.location.href = "/recruiter/settings";
    }
  };
          ...
          {showProfileMenu && (
            <>
              ...
              <div className="absolute right-0 top-12 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 p-1.5 space-y-0.5">
                ...
                <div className="p-2 border-t border-slate-100 bg-slate-50/80 rounded-xl space-y-1 my-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-1">Switch Test Account ID</p>
                  <button
                    onClick={() => switchTestAccount("VERIFIED")}
                    className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-800 hover:bg-emerald-100 transition-colors"
                  >
                    <span>🟢 verified@recruiter.com</span>
                    <span className="text-[9px] font-extrabold uppercase">Verified</span>
                  </button>
                  <button
                    onClick={() => switchTestAccount("UNFILLED")}
                    className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-semibold bg-amber-50 text-amber-800 hover:bg-amber-100 transition-colors"
                  >
                    <span>⏳ unverified@recruiter.com</span>
                    <span className="text-[9px] font-extrabold uppercase">Unverified</span>
                  </button>
                </div>
