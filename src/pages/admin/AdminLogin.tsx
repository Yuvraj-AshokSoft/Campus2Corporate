...
export const AdminLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback auth check via context or local env match if API server wasn't reachable
      if (!authSuccessful && !apiErrorMessage) {
        const contextAuth = await login(email.trim(), password);
