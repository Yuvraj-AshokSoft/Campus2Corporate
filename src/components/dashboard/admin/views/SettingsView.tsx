...
export const SettingsView: React.FC = () => {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Could not determine Admin role from profile:', profErr);
      }
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerToast(err.response?.data?.message || 'Failed to update system settings. Super Admin privileges required.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-8 font-sans text-slate-800 pb-12">
      ...
      {/* Super Admin Notice if ordinary Admin */}
      {!isSuperAdmin && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs font-bold text-amber-800 flex items-center gap-3">
          <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
          <span>
            Notice: Modifying platform-wide algorithms and system settings requires Super Admin privileges. Standard Administrators have read-only visibility.
          </span>
        </div>
      )}
