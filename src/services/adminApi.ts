...
// Response interceptor to handle 401 session expiry and distinguished 403 RBAC
adminApiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      // 403 Forbidden: differentiate inactive account / non-admin from Super Admin restriction
      if (status === 403) {
        const isAccountRevoked =
        ...
        // If it's a Super Admin requirement or other operation denial, do NOT wipe session or redirect
      }
    }
...
export const adminApi = {
  ...
  createContentRoadmap: async (data: {
    title: string;
    category?: string;
    description?: string;
    status?: string;
